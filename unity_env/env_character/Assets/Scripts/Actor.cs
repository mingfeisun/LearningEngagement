...
public class Actor : MonoBehaviour {
    ...
    */
    ...
    public Matrix4x4[] GetPosture() {
        ...
        for(int i=0; i<posture.Length; i++) {
            // posture[i] = Bones[i].Transform.GetWorldMatrix();
            posture[i] = Bones[i].Transform.localToWorldMatrix;
        }
        return posture;
    }
    ...
    public void DrawSimple(Color color, Matrix4x4[] transformations) {
        ...
        if(DrawSkeleton) {
            ...
            recursion = new Action<Bone>((bone) => {
                if(bone.GetParent() != null) {
                    UltiDraw.DrawLine(transformations[bone.GetParent().Index].GetColumn(3), transformations[bone.Index].GetColumn(3), color);
                }
                UltiDraw.DrawCircle(transformations[bone.Index].GetColumn(3), 0.02f, Color.Lerp(color, UltiDraw.Black, 0.25f));
                for(int i=0; i<bone.Childs.Length; i++) {
                    recursion(bone.GetChild(i));
                }
            });
