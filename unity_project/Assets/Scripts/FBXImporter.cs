...
public class FBXImporter : EditorWindow {
    ...
    [MenuItem ("Mocap Data/FBX Importer")]
    static void Init() {
        ...
        using(new EditorGUILayout.VerticalScope ("Box")) {
            ...
            using(new EditorGUILayout.VerticalScope ("Box")) {
                ...
                using(new EditorGUILayout.VerticalScope ("Box")) {
                    ...
                    EditorGUILayout.LabelField("Motions/", GUILayout.Width(50));
    ...
    private void LoadDirectory() {
        ...
        if(Directory.Exists(source)) {
            ...
            for(int i=0; i<items.Length; i++) {
                string path = items[i].FullName.Substring(items[i].FullName.IndexOf("Assets/Motions/"));
    ...
    private IEnumerator ImportMotionData() {
        string destination = "Assets/Motions";
        if(Character == null) {
            ...
            Matrix4x4[] initLocalMatrix = new Matrix4x4[Character.Bones.Length];
            Matrix4x4[] initWorldMatrix = new Matrix4x4[Character.Bones.Length];

            // store init pose
            for(int j=0; j<Character.Bones.Length; j++) {
                initLocalMatrix[j] = Character.Bones[j].Transform.worldToLocalMatrix;
                initWorldMatrix[j] = Character.Bones[j].Transform.localToWorldMatrix;
            }
                ...
                if(Files[f].Import) {
                    if(AssetDatabase.LoadAssetAtPath(destination+"/"+Files[f].Object.name+".asset", typeof(MotionData)) == null) {
                        ...
                        for(int i=0; i<data.GetTotalFrames(); i++) {
                          ...
                          for(int j=0; j<Character.Bones.Length; j++) {
                              data.Frames[i].Local[j] = Character.Bones[j].Transform.worldToLocalMatrix;
                              data.Frames[i].World[j] = Character.Bones[j].Transform.localToWorldMatrix;
                          }
                        }
            ...
            // reset to init pose
            for(int j=0; j<Character.Bones.Length; j++) {
                Character.Bones[j].Transform.position = initWorldMatrix[j].GetPosition();
                Character.Bones[j].Transform.rotation = initWorldMatrix[j].GetRotation();
            }
